...
* `/iexec_ou/computed.json` file should be created when your computing is over. It contains at least a field `deterministic-output-path` which contains the path of the deterministic portion of you results. It is required for the proof of execution \(given the same inputs this file should always be the same\).
